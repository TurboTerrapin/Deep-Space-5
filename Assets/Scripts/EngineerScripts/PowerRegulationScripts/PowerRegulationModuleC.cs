...
public class PowerRegulationModuleC : NetworkBehaviour, IControllable, IPowerRegulable, IIKTargetable
{
    ...
    IEnumerator stateChangeHelper(bool to_change_to)
    {
        ...
        float starting_rotation = prsc_crank.transform.localRotation.eulerAngles.z;
        ...
        while (anim_time > 0.0f)
        {
            ...
            prsc_crank.transform.localRotation = Quaternion.Euler(-54.0f, -45.0f, turn_rotation);

            yield return null;
        }

        prsc_display.SetActive(to_change_to);

        turn_configuration = 0;
        prsc_crank.transform.localRotation = Quaternion.Euler(-54.0f, -45.0f, 0.0f);
    ...
    private void displayAdjustment(int to_adjust, bool correct)
    {
        ...
        prsc_display.transform.GetChild(to_adjust).GetComponent<UnityEngine.UI.RawImage>().color = new Color(0.0f, 0.84f, 1.0f, a);
    }
    ...
    IEnumerator crankTurn(int prev_config, int to_turn_to)
    {
        ...
        while (anim_time > 0.0f)
        {
            ...
            prsc_crank.transform.localRotation =
    ...
    private void generateNewSequenceRPC(int turn_a, int turn_b, int turn_c, int turn_d, int turn_e)
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            prsc_display.transform.GetChild(i).GetComponent<UnityEngine.UI.RawImage>().color = new Color(0.0f, 0.84f, 1.0f, 0.2f);
            prsc_display.transform.GetChild(i).localRotation = Quaternion.Euler(0.0f, 0.0f, sequence_code[i] * 45.0f);
        }
